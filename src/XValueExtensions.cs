    ...
    public static class XValueExtensions
    {
        ...
        public static T xValue<T>(this object src, object @default = null)
        {
            ...
            if (typeof(T).IsEnum)
            {
                ...
                return (T)Enum.Parse(typeof(T), src.ToString(), ignoreCase: true);
            }

            if (typeof(T).xIsNumeric())
            {
            ...
            {
                return (T)Convert.ChangeType(time.xToDate(typeof(T) == typeof(int) ? ENUM_DATE_FORMAT.YYYYMMDD : ENUM_DATE_FORMAT.YYYY_MM_DD), typeof(T));
            }

            return (T)Convert.ChangeType(src, typeof(T));
        }
        
        public static bool xIsNumeric(this Type type)
        {
